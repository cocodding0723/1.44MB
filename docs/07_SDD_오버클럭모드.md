...
> **⚠️ SUPERSEDED (2026-06-29, 적대적검수 HIGH#4):** 위 `g_eneCap` 런타임 분리는 **구현되지 않았다**(코드엔 `MAXENE` 단일 상수만, `src/nd_data.inc`). 실제 구현은 모든 적 순회가 첫 줄 `if(!e->active) continue;`로 비활성 슬롯을 단락 → 본편은 동시 활성 적이 스폰 캡으로 자연히 적어 256 풀 순회의 실측 비용이 무시 가능. 별도 `g_eneCap` 없이도 본편 동작·체감 회귀 없음(코드리뷰 결정론·경계 통과). **결정: §14.6 활성캡 분리 폐기**(over-engineering 회피). 단 향후 본편에 대량 동시 적(>128)을 의도 도입하면 재검토. 프레임타임 정밀 prof는 실기 환경 제약으로 미실시.
